...
async function fetchEthPrice() {
    try {
        const res = await fetch(
            `https://api.g.alchemy.com/prices/v1/${process.env.ALCHEMY_API_KEY}/tokens/by-symbol`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
export async function GET() {
    try {
        ...
        // Fetch transfers for first 8 tokens only (to stay within rate limits)
        const transferPromises = TOKEN_LIST.slice(0, 8).map(t => fetchTransfers(t.address));
        const transferResults = await Promise.all(transferPromises);

        const coins = TOKEN_LIST.map((token, i) => {
            const addrKey = token.address.toLowerCase();
            const alchemyData = priceMap[addrKey];
            const change24h = changeMap[token.id] || 0;

            let price = 0;
            if (token.address.includes('eeeeeeee')) {
                price = ethPrice;
            } else {
                price = alchemyData?.price || 0;
            }
